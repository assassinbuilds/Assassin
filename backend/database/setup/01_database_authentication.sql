-- ==============================================================================
-- TechAssassin Database - Authentication and User Management
-- ==============================================================================
-- 
...
BEGIN
    ...
    session_token := encode(decode(md5(user_id::text || now()::text || random()::text), 'hex');
    
    -- Store session (you might want to create a sessions table for this)
    -- INSERT INTO public.sessions (user_id, token, created_at, expires_at)
    -- VALUES (user_id, session_token, NOW(), NOW() + INTERVAL '7 days');
...
-- ==============================================================================
-- USER PROFILE INSERTION FUNCTIONS
-- ==============================================================================
...
BEGIN
    ...
    -- Get user IDs
    SELECT id INTO user_id FROM public.profiles WHERE username = 'aryansondharva';
    ...
    (SELECT id FROM public.profiles WHERE username = 'sarahchen', event_id, 'Tech Titans', 'Mobile app for rural healthcare access', 'confirmed', NOW()),
    (SELECT id FROM public.profiles WHERE username = 'mikejohnson', event_id, 'Hack Masters', 'Blockchain-based donation tracking system', 'confirmed', NOW()),
    (SELECT id FROM public.profiles WHERE username = 'emilydavis', event_id, 'Design Thinkers', 'UX platform for skill-based volunteering', 'confirmed', NOW()),
    (SELECT id FROM public.profiles WHERE username = 'alexkumar', event_id, 'Algorithm Kings', 'AI-driven disaster response coordination', 'confirmed', NOW());
