...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (!userId) {
      ...
      router.push(`/signin?redirect_url=${encodeURIComponent(currentPath)}`);
      return;
    }
    ...
    fetchProfile();
  }, [username, isLoaded, userId, isOwnProfile, router]);
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          ...
          <Button onClick={() => router.push('/')} className="bg-slate-900 text-white rounded-full px-8 hover:bg-slate-800">
            Return to Home
          </Button>
        </div>
      </div>
                  ...
                  <div className="space-y-5">
                    ...
                    {!hasEducation && !(profile.address && profile.is_address_public) && !(profile.email && profile.is_email_public) && (
                      <div className="text-center py-6 bg-slate-50/50 border border-dashed border-slate-200 rounded-2xl">
                        ...
                        {isOwnProfile && (
                          <Button onClick={() => router.push('/edit-profile')} variant="outline" size="sm" className="rounded-xl text-[10px] font-black uppercase tracking-wider px-4 py-2 border-slate-200 hover:bg-slate-50 transition-colors">
                ...
                <div className="bg-white rounded-3xl border border-slate-200/60 p-6 sm:p-8 shadow-sm hover:shadow-md transition-all duration-300">
                  ...
                  {profile.bio ? (
                    ...
                    <div className="text-center py-10 bg-slate-50/50 border border-dashed border-slate-200 rounded-2xl">
                      ...
                      <Button onClick={() => router.push('/edit-profile')} variant="outline" size="sm" className="rounded-xl text-xs font-black uppercase tracking-wider">
